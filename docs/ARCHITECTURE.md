...
| Secrets | Electron **`safeStorage`** (Keychain-backed) | Never store Linear/Anthropic tokens in plaintext |
...
│   │   │   └── store/            # persisted app state (electron-store), safeStorage secrets
...
- Tokens encrypted with `safeStorage`, stored via `electron-store`.
...
### 4.6 The unified flow (v1 golden path)
