...
export default defineConfig(({ mode }) => {
  // Linear + Slack OAuth client credentials — read from a gitignored repo-root
  ...
  const env = loadEnv(mode, resolve(__dirname, '../..'), ['LINEAR_', 'SLACK_']);
  const oauthDefine = {
    ...
    'process.env.SLACK_CLIENT_ID': JSON.stringify(env.SLACK_CLIENT_ID ?? ''),
    'process.env.SLACK_CLIENT_SECRET': JSON.stringify(env.SLACK_CLIENT_SECRET ?? ''),
  };
